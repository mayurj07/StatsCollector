...
public class StatExtractor {
		...
		public String[] PerfCounters = { "cpu.usage.average", "mem.usage.average", "net.usage.average", "disk.usage.average" };
		...
		StatExtractor(ManagedEntity vm) throws RemoteException, IOException
        {
			this.vm = vm;
			continueProgram();
		}
		...
		public void continueProgram() {
			perfMgr = GatherStat.si.getPerformanceManager();
			PerfCounterInfo[] pcis = perfMgr.getPerfCounter();

			// create map between counter ID and (PerfCounterInfo, counter name, ID)
		...
		private void printPerfMetricCSV(PerfEntityMetricCSV pem) throws FileNotFoundException {
			...
			try {
				...
				for (String counter : PerfCounters)
                {
					...
					// str.append(pci.getGroupInfo().getKey() + "."
					 //+ pci.getNameInfo().getKey() + "." +
				 //pci.getRollupType()).append("=");
					//str.append(",");
		...
		synchronized private PerfQuerySpec createPerfQuerySpec(ManagedEntity me, int maxSample, int interval) {
		...
		public void setCurrentLog(String currentLog) {  currentLog = currentLog;    }
